...
"249968",
        ...
        public void Ping(string domain)
        {
            ...
            try
            {
                IPAddress ipAddress = Dns.GetHostEntry("r40-" + textBox1.Text + domain).AddressList[0];
                Ping ping = new Ping();
       ...
       async private void button2_Click(object sender, EventArgs e)
        ...
        public void Async_Power_Shell_Service_Rebut(string ip, string name_service, ProgressBar progressBar, TextBox textBox)
        {
            try
            {
                string check_action;
                progressBar.Value = 40;
                Powershell_service(ip, "Stopped", ReNameService(name_service), "", out check_action);
                if (check_action == "\r\nStopped\r\n\r\n\r\n")
                {
                    progressBar.Value = 60;
                    Powershell_service(ip, "Running", ReNameService(name_service), "", out check_action);
                    progressBar.Value = 100;
                    MessageBox.Show($"Служба \"{name_service}\"\nНа компьютере {ip} - Перезапущена\n");
                    progressBar.Value = 0;
                    textBox.Text = check_action.Replace("\r\n", "");
        ...
        public string[] Status_all_Service()
        {
            try
            {
                ...
                {
                    FileName = "powershell",
                    Arguments = $"/command get-service -DisplayName Russian.Post* -ComputerName {p} | format-table Status -autosize",
                    UseShellExecute = false,
                    CreateNoWindow = true,
                    RedirectStandardOutput = true
                });
                string status_service = process.StandardOutput.ReadToEnd().Substring(18);
                string[] mass_status = new string[12];
                ...
                string[] mass_name_service = new string[]{ "Russian.Post.EAS4.Cart",
                        "Russian.Post.EAS4.Cash", "Russian.Post.EAS4.Fiscal", "Russian.Post.EAS4.PinPad", "Russian.Post.MARS.Discovery", "Russian.Post.MARS.Downloader",
                "Russian.Post.MARS.Launcher","Russian.Post.MARS.NSS","Russian.Post.MARS.PerformanceCounter","Russian.Post.MARS.Replicator","Russian.Post.MARS.Updater",
                "Russian.Post.MARS.WatchDog"};
       ...
       () =>
       {
           Start_ALL_Service(progressBar1, textBox3);
                    });
                thread.Start();
            }
            else
        ...
        private void button15_Click(object sender, EventArgs e)
        {
            if (textBox3.Text != "")
            {
                ...
                Thread thread = new Thread(
                    () =>
                    {
                        Stop_Service_All(progressBar1, textBox3);
                    });
                thread.Start();
            }
            else
        ...
        private void button14_Click_1(object sender, EventArgs e)
        {
            if (textBox3.Text != "")
            {
                ...
                Thread thread = new Thread(
                    () =>
                    {
                        Rebut_MPK_Service(progressBar1);
                    });
                thread.Start();
            }
            else
